...
async def timeattack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or user.lower() != OWNER_USERNAME.lower():
        await update.message.reply_text("မင်းမသုံးနိုင်ဘူးကုလား")
        return
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not is_authorized(f"@{user}"):
        await update.message.reply_text("Spam BotပထမဦးဆုံးBotကိုOwner အမိန့်မပါပဲသုံးခြင်တာလား၀က်")
        return
    ...
    for raw_target in context.args:
        ...
        if isinstance(target, str) and target.lower() == OWNER_USERNAME_LC:
            if attacker.lower() == OWNER_USERNAME_LC:
                ...
                await update.message.reply_text(f"အရှင်သခင်ကိုလုပ်လို့မရပါအရှင်သခင့်ကိုတိုက်တဲ့ကောင်ကိုစတင်သုတ်သင်ပြီကုလားတွေ")
        ...
        if attacker_lower not in [a.lower() for a in ADMIN_USERNAMES] and attacker_lower != OWNER_USERNAME_LC:
            ...
            if remaining <= 0:
                await update.message.reply_text(f"Attack uses မကျန်တော့ပါ။ ပိုင်ရှင်ဆီထပ်မံ၀ယ်ယူပါ")
                continue
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not is_authorized(f"@{user}"):
        await update.message.reply_text("Owner အမိန့်မရှိဘဲမင်းစောက်ဆင့်နဲ့သုံးခြင်နေသေးတယ်")
        return
    ...
    if arg == "all":
        ...
        await update.message.reply_text("ခွေးအားလုံးအပေါ် သုတ်သင်ခြင်းရပ်တန့်လိုက်ပြီ")
        return
    ...
    if user_lower not in [a.lower() for a in ADMIN_USERNAMES] and user_lower != OWNER_USERNAME_LC:
        ...
        if remaining <= 0:
            await update.message.reply_text(f"Stop uses မကျန်တော့ပါပိုင်ရှင်ဆီဆက်သွယ်ပါ")
            return
...
# ===== /buy command =====
# -----------------------------
# /buy command with new button
# -----------------------------
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    default_target = "problem_zenki"  # default target, လိုသလိုပြောင်းနိုင်

    # Save default target for user
    purchase_data[user_id] = {"feature": "", "target": default_target, "qty": 0, "paid": True, "tx_id": ""}

    # Feature buttons + New Buy Full Guide button
    keyboard = [
        [InlineKeyboardButton("Attack+Stop", callback_data='feature_attack')],
        [InlineKeyboardButton("Hell+Stophell", callback_data='feature_hell')],
        [InlineKeyboardButton("Funny+Stopfunny", callback_data='feature_funny')],
        [InlineKeyboardButton("ဘော့လုပ်နည်း Full Guide", callback_data='buy_full_guide')]  # အသစ်ထည့်
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        "အသုံးချချင်တဲ့ God feature ကိုရွေးပါ:", reply_markup=reply_markup
    )


# -----------------------------
# Feature button callback
# -----------------------------
async def feature_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id

    # Buy Full Guide button ကို handle
    if query.data == "buy_full_guide":
        await query.edit_message_text(
            "📘 ဘော့လုပ်နည်း Full Guide\n"
            "💰 စျေးနှုန်း = 15,000 Kyat\n"
            "📩 ဆက်သွယ်ရန် - @Problem_Zenki ကို Forward လုပ်ပြီး စာပို့လိုက်ပါ။\n"
            "⚠️ လူဆယ်ယောက်ပဲ သင်ပေးမှာ ဖြစ်ပါတယ်။"
        )
        return

    # ရှိပြီးသား feature selection logic
    if user_id not in purchase_data:
        await query.edit_message_text("အမှားတက်နေပါတယ်။ /buy command ကို အသုံးပြုပါ။")
        return

    feature = query.data.split("_")[1]  # attack / hell / funny
    purchase_data[user_id]["feature"] = feature
    await query.edit_message_text(
        f"{feature.capitalize()} ကို ရွေးပြီးပါပြီ။ ၁–၂၀ ကြိမ်အတွင်း ရွေးချယ်ပါ။"
    )
...
async def main():
    ...
    app.add_handler(CallbackQueryHandler(feature_select))
