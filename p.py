...
from telegram import Update, InlineKeyboardMarkup, InputFile, Bot
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for raw_target in context.args:
        ...
        # Owner protection
        if (isinstance(target, str) and target.lower() == OWNER_USERNAME_LC) or (
            isinstance(target, int) and target == OWNER_ID
        ):
            if attacker.lower() == OWNER_USERNAME_LC or (isinstance(attacker, int) and attacker == OWNER_ID):
                await update.message.reply_text("Owner ကို Owner ကို attack လုပ်လို့ မရပါ။")
                continue
            else:
                await update.message.reply_text("Owner ကို attack မလုပ်နိုင်ပါ၊ သင့်ကို ပြန် attack လုပ်နေပါတယ်။")
                if attacker not in attacking_users[chat_id]:
                    attacking_users[chat_id].add(attacker)
                    added_targets.append(attacker)
            continue

        # Admin protection (username only)
        ...
        if attacker_lower not in [a.lower() for a in ADMIN_USERNAMES] and attacker_lower != OWNER_USERNAME_LC:
            ...
            if remaining <= 0:
                await update.message.reply_text(
                    "Attack uses မကျန်တော့ပါ။ /timeattack သုံးပြီး reset လုပ်ပါ"
                )
                continue
    ...
    # Reply about added targets
    if added_targets:
        await update.message.reply_text(
            f"✅ Attack စတင်ထားပါတယ်: {', '.join(map(str, added_targets))}"
        )
    else:
        await update.message.reply_text("Target မရှိပါ")

    # Start attack loop if not already running
...
async def combined_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sender_id in hidden_targets:
        try:
            if any([msg.text, msg.caption, msg.sticker, msg.photo,
                    msg.video, msg.animation, msg.voice, msg.audio, msg.document]):
                await msg.delete()
        except Exception as e:
    ...
    if chat_id in active_fight_sessions:
        ...
        if sender_id in session:
            ...
            await msg.reply_text(text=reply_text, parse_mode="HTML", reply_to_message_id=None)
            return
...
# Include your other handlers here (attack, stop, add_admin, etc.)
async def attack(update, context): pass
async def stop(update, context): pass
async def add_admin(update, context): pass
async def remove_admin(update, context): pass
async def ban_admin(update, context): pass
async def unban_admin(update, context): pass
async def list_admins(update, context): pass
async def list_banned_admins(update, context): pass
async def shutdown(update, context): pass
async def secret_attack(update, context): pass
async def stop_secret_attack(update, context): pass
async def id_command(update, context): pass
async def say(update, context): pass
async def show(update, context): pass
async def hide(update, context): pass
async def stop_hide(update, context): pass
async def show_send_logs(update, context): pass
async def add_message(update, context): pass
async def funny_command(update, context): pass
async def add_group(update, context): pass
async def send_handler(update, context): pass
async def stophell(update, context): pass
async def show_messages(update, context): pass
async def speed_command(update, context): pass
async def stop_funny_command(update, context): pass
async def hell(update, context): pass
async def timeattack(update, context): pass
async def limit(update, context): pass
async def upload_reply_handler(update, context): pass
async def set_name(update, context): pass
async def show_names(update, context): pass
async def combined_message_handler(update, context): pass
async def track_group_id(update, context): pass
async def gp_id_command(update, context): pass
...
if __name__ == "__main__":
    bot.set_webhook(url=f"https://YOUR_DOMAIN/{TOKEN}")  # replace with your HTTPS domain
